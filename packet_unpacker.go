...
func (u *packetUnpacker) Unpack(headerBinary []byte, hdr *wire.Header, data []byte) (*unpackedPacket, error) {
	...
	for r.Len() > 0 {
		...
		} else if u.version.CryptoStreamID() == 0 && typeByte == 0x4 { // in IETF QUIC, 0x4 is a MAX_DATA frame
		...
		} else if u.version.CryptoStreamID() == 0 && typeByte == 0x5 { // in IETF QUIC, 0x5 is a MAX_STREAM_DATA frame
		...
		} else if u.version.CryptoStreamID() == 0 && typeByte == 0x8 { // in IETF QUIC, 0x4 is a BLOCKED frame
			frame, err = wire.ParseBlockedFrame(r, u.version)
		} else if u.version.CryptoStreamID() == 0 && typeByte == 0x9 { // in IETF QUIC, 0x4 is a STREAM_BLOCKED frame
			frame, err = wire.ParseBlockedFrameLegacy(r, u.version)
			if err != nil {
				err = qerr.Error(qerr.InvalidBlockedData, err.Error())
			}
