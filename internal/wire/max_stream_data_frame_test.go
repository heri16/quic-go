...
var _ = Describe("MAX_STREAM_DATA frame", func() {
	Context("parsing", func() {
		It("accepts sample frame", func() {
			...
			frame, err := ParseMaxStreamDataFrame(b, versionCryptoStream0)
		...
		It("errors on EOFs", func() {
			...
			_, err := ParseMaxStreamDataFrame(bytes.NewReader(data), versionCryptoStream0)
			Expect(err).NotTo(HaveOccurred())
			for i := range data {
				_, err := ParseMaxStreamDataFrame(bytes.NewReader(data[0:i]), versionCryptoStream0)
