...
var _ = Describe("MAX_DATA frame", func() {
	Context("when parsing", func() {
		...
		It("errors on EOFs", func() {
			...
			_, err := ParseMaxDataFrame(bytes.NewReader(data), versionCryptoStream0)
			Expect(err).NotTo(HaveOccurred())
			for i := range data {
				_, err := ParseMaxDataFrame(bytes.NewReader(data[0:i]), versionCryptoStream0)
	...
	Context("writing", func() {
		It("has proper min length", func() {
			...
			Expect(f.MinLength(versionCryptoStream0)).To(Equal(protocol.ByteCount(1 + 8)))
		})
		...
		It("writes a MAX_DATA frame", func() {
			...
			err := f.Write(b, versionCryptoStream0)
			Expect(err).ToNot(HaveOccurred())
