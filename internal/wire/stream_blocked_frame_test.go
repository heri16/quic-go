...
var _ = Describe("STREAM_BLOCKED frame", func() {
	Context("parsing", func() {
		It("accepts sample frame", func() {
			...
			frame, err := ParseStreamBlockedFrame(b, versionCryptoStream0)
		...
		It("errors on EOFs", func() {
			...
			_, err := ParseStreamBlockedFrame(bytes.NewReader(data), versionCryptoStream0)
			Expect(err).NotTo(HaveOccurred())
			for i := range data {
				_, err := ParseStreamBlockedFrame(bytes.NewReader(data[0:i]), versionCryptoStream0)
